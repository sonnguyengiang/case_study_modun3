...
public class CRUD_Detail_Bill {
    ...
    public static void CreateDetailBill(Detail_Bill detail_bill) throws SQLException {
        String create = "insert into detail_bill(id_user, id_sp, count_price, soluongmua) value (?, ? , ? ,?)";
        PreparedStatement preparedStatement = connection.prepareStatement(create);
        preparedStatement.setInt(1, detail_bill.getId_user());
        preparedStatement.setInt(2, detail_bill.getId_sp());
        preparedStatement.setInt(3, detail_bill.getTong_gia());
        preparedStatement.setInt(4, detail_bill.getSoluongmua());
        preparedStatement.execute();
    }

    public static ArrayList<UserBill> getBillTemp(int id_user) throws SQLException {
        String select = "select detail_bill.id_detail_bill, product.id_sp, product.img, product.name_sp, product.price, product.soluong, detail_bill.count_price from product inner join detail_bill on product.id_sp = detail_bill.id_sp where detail_bill.id_user = " + id_user + "";
